...
resource "local_file" "workers-pem" {
  ...
  content         = tls_private_key.workers[count.index].private_key_openssh
...
resource "outscale_vm" "workers" {
  ...
  user_data = base64encode(format("#cloud-config\nssh:\n  emit_keys_to_console: false\nssh_keys:\n  ed25519_private: |\n    %s\n  ed25519_public: %s",
    indent(4, tls_private_key.workers-sshd[count.index].private_key_openssh),
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      private_key         = tls_private_key.workers[count.index].private_key_openssh
